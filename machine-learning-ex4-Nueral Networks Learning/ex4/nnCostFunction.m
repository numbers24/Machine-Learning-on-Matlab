...
%
    % very important rule: a' * b = b' * a
    %create vector K where K = [1,2,3,...num_Labels]
    K = zeros(num_labels,1);
    for k = 1:num_labels
        K(k)= k;
    end
    %create Boolean Matrix Y which will hold every y sub k
    
    Y = y == K';
    
    %apply h to all X and store it in H
